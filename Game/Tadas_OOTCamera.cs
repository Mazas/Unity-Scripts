...
public class Tadas_OOTCamera : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        speed = pContr.speed;
        Vector3 behind = player.transform.position - new Vector3(player.transform.forward.x * distance, player.transform.forward.y - 3.0f, player.transform.forward.z * distance);
    ...
    void Update()
    {
       ...
       // CheckCameraRotation();
        ...
        {
            transform.LookAt(player.transform);
            transform.position = Vector3.MoveTowards(transform.position,behind, speed * 2.7f * Time.deltaTime);
        }
